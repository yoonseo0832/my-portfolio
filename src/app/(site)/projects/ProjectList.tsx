...
export default function ProjectList({ projects }: { projects: ProjectItem[] }) {
  ...
  const visible = projects.filter(
    (p) => filter === "all" || (p.platform ?? "github") === filter,
  );
  return (
    <div className="projects-list">
      <div className="platform-filters">
        ...
        {platforms.map((platform) => (
          <button
            key={platform.id}
            className={filter === platform.id ? "active" : ""}
            onClick={() => setFilter(platform.id)}
          >
            {platform.label}{" "}
            <span>
              {
                projects.filter((p) => (p.platform ?? "github") === platform.id)
                  .length
              }
            </span>
          </button>
          ...
          <Card key={p.id} className="project-card">
            <CardHeader>
              <Badge variant="outline" className="source-badge">
                {p.platformLabel ?? "GitHub"}
              </Badge>
              <CardTitle>{p.title}</CardTitle>
              <CardDescription className="project-date">
                {new Intl.DateTimeFormat("ko-KR").format(new Date(p.updatedAt))}
              </CardDescription>
            </CardHeader>
            <CardContent>
      ...
      {visible.length === 0 && (
        <p className="projects-empty">해당 플랫폼의 프로젝트가 없습니다.</p>
      )}
    </div>
  );
}
